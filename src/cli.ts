...
prog
  .option(
    "-c, --chars <characters>",
    "The list of characters you have in any order"
  )
  .option(
    "-p, --pattern <pattern>",
    "a pattern to match, if you know the positions. Eg: al**t"
  )
  .option(
    "-l, --list <word-list-url>",
    "a link to a plain text list of words split by newline. Optional"
  )
  .option(
    "-b, --blacklist <chars>",
    "a list of characters to blacklist. Optional"
  );
...
async function main() {
  ...
  const { chars, list, pattern, blacklist } = prog.opts();
  ...
  if (chars) {
    matches = findMatchingWords(chars.toLowerCase(), wordList, blacklist);
  }
